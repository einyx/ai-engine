use airproxy_provider::anthropic::{MessageContent, MessagesRequest};
use airproxy_provider::openai::{ChatContent, ChatRequest, EmbeddingsInput, EmbeddingsRequest};
...
fn openai_chat_content_string_form() {
    ...
    let m: airproxy_provider::openai::ChatMessage = serde_json::from_str(raw).unwrap();
...
fn anthropic_message_content_string_and_blocks() {
    ...
    let m: airproxy_provider::anthropic::Message = serde_json::from_str(raw_str).unwrap();
    assert!(matches!(m.content, MessageContent::Text(ref s) if s == "hi"));

    let raw_blocks = r#"{"role": "user", "content": [{"type": "text", "text": "hi"}]}"#;
    let m: airproxy_provider::anthropic::Message = serde_json::from_str(raw_blocks).unwrap();
