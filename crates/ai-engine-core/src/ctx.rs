use ai_engine_provider::{anthropic, error::ProviderError, openai};
use bytes::Bytes;
...
pub struct ProviderBinding {
    /// All providers that serve the requested model, in route order. The
    /// `forward` stage load-balances across these and may fail over between
    /// them.
    pub candidates: Vec<String>,
    /// The provider `forward` actually dispatched to. Empty until `forward`
    /// resolves it; read by the log stage so it reflects the real upstream.
