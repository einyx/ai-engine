...
use crate::partition::{agreed_manifest, manual_partition, PartitionManifest};
...
pub struct ClusterLeader {
    ...
    pub(crate) metrics: std::sync::Arc<crate::metrics::ClusterMetrics>,
}
...
impl ClusterLeader {
    ...
    pub async fn start(identity: &NodeIdentity, cfg: LeaderConfig) -> anyhow::Result<Self> {
        ...
        let manifest = if let Some(ranges) = cfg.partition_override.clone() {
            ...
            // Coordinator-free agreement: canonicalise the capability set so a
            // worker recomputing from the same peers derives an identical hash.
            agreed_manifest(
                &cfg.model_id,
                &capabilities,
                cfg.n_layers,
        ...
        Ok(Self {
            ...
            metrics: std::sync::Arc::new(crate::metrics::ClusterMetrics::new()),
        })
    }
    ...
    /// Capabilities collected from workers at join time.
    pub fn capabilities(&self) -> &[Capability] {
        &self.capabilities
    }

    /// Shared token counter for the metrics endpoint.
    pub fn metrics(&self) -> std::sync::Arc<crate::metrics::ClusterMetrics> {
        self.metrics.clone()
    }
