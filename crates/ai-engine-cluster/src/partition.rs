...
impl PartitionManifest {
    ...
    /// The node holding the token-embedding layer (pipeline head). A
    /// coordinator sends prompt token ids here to start a forward pass.
    pub fn embedding_host(&self) -> Option<&str> {
        self.assignments
            .iter()
            .find(|a| a.hosts_embedding)
            .map(|a| a.node_id.as_str())
    }

    /// The node holding the final norm + output projection (pipeline tail).
    /// A coordinator reads logits back from here to sample the next token.
    pub fn output_host(&self) -> Option<&str> {
        self.assignments
            .iter()
            .find(|a| a.hosts_output)
            .map(|a| a.node_id.as_str())
    }

    /// The ring sequence from embedding head to output tail, following
    /// `next_node`. This is the order any node — leader or not — drives a
    /// forward pass through; it does not depend on who is coordinating.
    ///
    /// Returns `None` if the chain is broken (missing/cyclic `next_node`),
    /// which would indicate a malformed manifest.
    pub fn pipeline_order(&self) -> Option<Vec<&str>> {
        let head = self.assignments.iter().find(|a| a.hosts_embedding)?;
        let mut order = Vec::with_capacity(self.assignments.len());
        let mut cur = Some(head.node_id.as_str());
        ...
        while let Some(id) = cur {
            ...
            order.push(id);
            let a = self.for_node(id)?;
            cur = a.next_node.as_deref();
        }
        // A well-formed chain visits every node and ends at the output host.
        if order.len() != self.assignments.len() {
            return None;
        }
        Some(order)
    }

    /// The forward-pass plan a coordinator on `local_id` executes: the pipeline
    /// ring with each hop flagged `local` (run in-process on this node) or not
    /// (RPC the activations to that peer over the mesh). This is what makes any
    /// node a valid entry point — the plan is identical regardless of who runs
    /// it, only the `local` flags differ per coordinator.
    pub fn forward_plan(&self, local_id: &str) -> Option<Vec<Step>> {
...
/// One hop in a coordinator's forward pass.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Step {
    pub node_id: String,
    /// True when this node's layer range runs in-process on the coordinator;
    /// false when the coordinator must RPC the activations to a mesh peer.
    pub local: bool,
}
...
/// Coordinator-free partition: canonicalises the capability set (total order
/// by `node_id`) before solving, so any node that discovers the same set of
/// peers — in any order — computes a byte-identical manifest and therefore an
/// identical `model_config_hash`. This is the primitive that lets nodes agree
/// on a plan by comparing hashes instead of trusting a leader to dictate one.
///
/// Any deterministic total order would do; `node_id` is used because every node
/// already knows it for every peer. Ordering does not change the *amount* of
/// work assigned (the DP balances on compute regardless) — only the ring
/// sequence and which ends host embedding/output.
pub fn agreed_manifest(
    model_id: &str,
    caps: &[Capability],
    n_layers: usize,
    layer_bytes: u64,
    ...
    let mut canonical = caps.to_vec();
    canonical.sort_by(|a, b| a.node_id.cmp(&b.node_id));
    auto_partition(
        model_id,
        &canonical,
        n_layers,
...
/// Two nodes agree on a plan iff their manifests hash identically. The hash
/// covers `(model_id, n_layers, [(node_id, layer_range)])`, so equal hashes
/// mean an identical layer assignment.
pub fn manifests_agree(a: &PartitionManifest, b: &PartitionManifest) -> bool {
    a.model_config_hash == b.model_config_hash
}
