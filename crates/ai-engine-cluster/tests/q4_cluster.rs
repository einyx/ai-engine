//! Q4 cluster generation matches Q4 single-node baseline.
//!
//! Spins up a leader + 2 workers loading the toy-llama-3-q4 fixture, runs
//! 5-token greedy generation across the cluster, and asserts the output
//! matches a single-node Q4 baseline exactly. Greedy + lossless wire
//! serialization should produce identical tokens.
...
async fn q4_cluster_generation_matches_q4_single_node() {
    ...
    // Single-node Q4 baseline.
    let baseline = single_node_q4_greedy_5(&fix, &cfg, &ids_i32);

    // 3-node cluster: leader hosts no layers, w1 = 0..2, w2 = 2..4.
