...
//! key (or the worker reused state across keys), the second request's logits
//! would diverge from its single-node baseline.
//!
//! Each call to `ClusterLeader::full_forward_for_test` allocates a fresh
//! `Uuid::now_v7()`, so we run three back-to-back forwards on the *same*
//! leader and the *same* worker tasks, each with a different prompt, and
//! check each one against its respective single-node baseline. If the
//! worker's per-request cache map is buggy, at least one of the comparisons
//! will fail.
//!
//! Sequential rather than concurrent: the leader's `full_forward_for_test`
//! is `&mut self`, and the workers' current accept loop accepts a single
//! inbound connection, so two leader instances can't both connect. Sequential
//! exercises the contract that matters — distinct `request_id`s do not leak
//! state across requests on the same connection.
...
fn single_node_logits(model_path: &Path, cfg: &ModelConfig, ids_i32: &[i32]) -> Vec<f32> {
...
async fn distinct_request_ids_do_not_leak_kv_state() {
    ...
    // Compute single-node baselines for each prompt.
    let baselines: Vec<(Vec<i32>, Vec<f32>)> = prompts
        .iter()
        .map(|p| {
            let ids: Vec<u32> = tok.encode(p).unwrap();
            let ids_i32: Vec<i32> = ids.iter().map(|x| *x as i32).collect();
            let logits = single_node_logits(&model_path, &cfg, &ids_i32);
            (ids_i32, logits)
        })
        .collect();
    ...
    // worker tasks. Each forward gets a distinct request_id via
    ...
    for (i, (ids_i32, baseline)) in baselines.iter().enumerate() {
        ...
        eprintln!("request {i}: max diff = {diff}");
        assert!(
            diff < 1e-3,
            "request {i}: cluster logits should match single-node baseline within 1e-3 (got {diff})"
        );
    }
}
