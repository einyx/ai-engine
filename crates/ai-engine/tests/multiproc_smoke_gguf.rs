//! Multi-process smoke test for GGUF binary wiring (v0.3.0-alpha.5).
//!
//! Same shape as `multiproc_smoke.rs`, but the cluster's `weights_path` points
//! at the `.gguf` toy fixture. Proves that `load_weights` dispatches correctly
//! end-to-end via the binary path: workers + leader load GGUF, generate tokens,
//! and the HTTP endpoint returns a well-formed OpenAI response.
//!
