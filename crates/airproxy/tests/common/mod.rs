//! Shared test harness: spin up an airproxy bound to 127.0.0.1:0 with a config
...
/// Spawn airproxy in the background, returning its base URL like "http://127.0.0.1:PORT".
pub async fn spawn(cfg_toml: &str) -> String {
    let cfg = Config::from_str(cfg_toml).expect("config parse");
    let state: Arc<airproxy_http::AppState> =
        airproxy::app::build_app_state(&cfg).expect("app state");
