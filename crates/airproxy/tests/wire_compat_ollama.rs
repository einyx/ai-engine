...
/// won't tolerate one, so airproxy must omit it when api_key is unset.
struct NoAuthHeader;
...
async fn ollama_no_api_key_works_end_to_end() {
    ...
    // Build airproxy config WITHOUT api_key on the openai-kind provider (Ollama scenario).
    let cfg = common::config_for("openai", &upstream.uri(), /*with_api_key=*/ false);
    let gw_base = common::spawn(&cfg).await;

    // Hit airproxy WITHOUT sending an Authorization header. Passthrough auth records
    ...
    assert_eq!(
        resp.status().as_u16(),
        200,
        "Ollama-shaped request through airproxy succeeds"
    );
...
async fn ollama_via_async_openai_sdk() {
    // Equivalent test using the async-openai SDK against airproxy → Ollama mock.
    // The SDK always sends Authorization; airproxy forwards it (passthrough),
    ...
    let resp = client
        ...
        .create(req)
        .await
        .expect("SDK call against airproxy → Ollama mock");
