...
use airproxy_stages::model_route::ModelRouteStage;
use http::HeaderMap;
...
async fn first_matching_rule_wins() {
    ...
    assert_eq!(c.binding.as_ref().unwrap().provider_id, "exact");
}
...
async fn glob_matches_when_no_exact() {
    ...
    s.process(&mut c).await.unwrap();
    assert_eq!(c.binding.as_ref().unwrap().provider_id, "glob");
}
...
async fn upstream_model_override_applied() {
    ...
    assert_eq!(b.provider_id, "openai");
