...
fn record_openai_usage(slot: &Option<airproxy_core::ctx::UsageSlot>, u: &openai::Usage) {
    if let Some(s) = slot {
...
fn record_anthropic_usage(
    slot: &Option<airproxy_core::ctx::UsageSlot>,
    u: &airproxy_provider::anthropic::AnthropicUsage,
) {
    if let Some(s) = slot {
        if let Ok(mut g) = s.lock() {
            *g = Some(RecordedUsage {
...
/// event unchanged.
fn tap_openai_usage(
    stream: airproxy_provider::provider::EventStream<openai::ChatStreamEvent>,
    slot: Option<airproxy_core::ctx::UsageSlot>,
...
fn tap_anthropic_usage(
    stream: airproxy_provider::provider::EventStream<airproxy_provider::anthropic::MessagesEvent>,
    slot: Option<airproxy_core::ctx::UsageSlot>,
) -> impl futures::Stream<Item = Result<StreamItem, ProviderError>> + Send + 'static {
    stream.map(move |ev| match ev {
        Ok(e) => {
