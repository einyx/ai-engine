...
// ── rustyllm provider tests ───────────────────────────────────────────────

fn rustyllm_provider_toml(weights_line: &str) -> String {
    format!(
        r#"{}
[[provider]]
id = "local"
...
{}
"#,
        CANDLE_BASE, weights_line
    )
}

#[test]
fn parse_rustyllm_provider_with_model_dir() {
    let toml = rustyllm_provider_toml(r#"weights_path = "/srv/models/Llama-3.2-1B""#);
    let cfg = Config::from_str(&toml).unwrap();
    let p = &cfg.providers[0];
    assert_eq!(p.kind, "rustyllm");
    assert_eq!(p.weights_path.as_deref(), Some("/srv/models/Llama-3.2-1B"));
}

#[test]
fn rustyllm_requires_weights_path() {
    let toml = rustyllm_provider_toml("");
    let err = Config::from_str(&toml).unwrap_err().to_string();
    assert!(err.contains("weights_path"), "got: {err}");
}

#[test]
fn rustyllm_rejects_gguf_weights() {
    let toml = rustyllm_provider_toml(r#"weights_path = "/models/llama.gguf""#);
    let err = Config::from_str(&toml).unwrap_err().to_string();
    assert!(err.to_lowercase().contains("gguf"), "got: {err}");
}
