...
pub fn validate(cfg: &Config) -> anyhow::Result<()> {
    ...
    for p in &cfg.providers {
        ...
        if !matches!(
            p.kind.as_str(),
            "openai" | "anthropic" | "local-cluster" | "candle" | "rustyllm"
        ) {
            anyhow::bail!("unknown provider kind `{}` (provider `{}`); expected `openai`, `anthropic`, `local-cluster`, `candle`, or `rustyllm`", p.kind, p.id);
        }
    }

    // candle provider validation
    for p in &cfg.providers {
        if p.kind == "candle" {
            ...
            if !weights.ends_with(".gguf") {
                anyhow::bail!(
                    "provider '{}': candle requires a .gguf weights_path, got '{}'",
                    p.id,
                    weights
                );
            }
    ...
    // rustyllm provider validation: needs an HF-safetensors model dir or
    // hub repo id (not a GGUF file).
    for p in &cfg.providers {
        if p.kind == "rustyllm" {
            let weights = p.weights_path.as_deref().unwrap_or("");
            if weights.is_empty() {
                anyhow::bail!(
                    "provider '{}': rustyllm requires a weights_path (model directory or hub repo id)",
                    p.id
                );
            }
            if weights.ends_with(".gguf") {
                anyhow::bail!(
                    "provider '{}': rustyllm loads HF safetensors, not GGUF; weights_path '{}' should be a model dir or hub id",
                    p.id,
                    weights
                );
            }
