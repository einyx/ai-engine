use ai_engine_core::ctx::{Identity, ProviderBinding, RecordedUsage, RequestBody, RequestCtx};
...
use ai_engine_stages::log::{LogSink, LogStage};
use http::HeaderMap;
...
fn ctx_full() -> (RequestCtx, Arc<CaptureSink>) {
    ...
    ctx.binding = Some(ProviderBinding {
        candidates: vec!["openai-prod".into()],
