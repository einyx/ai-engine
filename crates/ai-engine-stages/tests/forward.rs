use ai_engine_core::ctx::{ProviderBinding, RequestBody, RequestCtx, ResponseSlot};
...
use ai_engine_stages::ProviderRegistry;
use ai_engine_stages::forward::ForwardStage;
use futures::stream;
use http::HeaderMap;
use std::sync::Arc;
...
fn make_ctx(stream: bool) -> RequestCtx {
    ...
    ctx.binding = Some(ProviderBinding {
        candidates: vec!["mock-openai".into()],
...
fn make_stage(provider: Arc<dyn Provider>) -> ForwardStage {
    let providers = make_registry(provider);
    let tracker = std::sync::Arc::new(ai_engine_stages::LoadTracker::new(
        providers.ids().cloned().collect::<Vec<_>>(),
    ));
    let metrics = std::sync::Arc::new(ai_engine_core::metrics::GatewayMetrics::new(
        providers.ids().cloned().collect::<Vec<_>>(),
    ));
    ForwardStage {
        providers,
...
async fn missing_binding_produces_internal_error() {
    ...
    assert!(matches!(
        err.error,
        ai_engine_core::error::GatewayError::Internal(_)
    ));
}
