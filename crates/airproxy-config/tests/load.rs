...
fn env_interpolation_substitutes_known_vars() {
    std::env::set_var("AIRPROXY_TEST_KEY_OK", "sk-substituted");
    let toml = r#"
[server]
bind = "127.0.0.1:0"
...
[auth]
mode = "shared-key"
master_keys = [{ key = "${AIRPROXY_TEST_KEY_OK}", name = "default" }]
...
[auth]
mode = "shared-key"
master_keys = [{ key = "${AIRPROXY_DEFINITELY_NOT_SET_ZZZ}", name = "default" }]
[[provider]]
id = "p"
kind = "openai"
base_url = "x"
