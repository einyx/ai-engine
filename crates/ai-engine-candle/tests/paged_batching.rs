...
fn run_engine(path: &Path, prompts: &[&str], cap: usize) -> Vec<Vec<u32>> {
    let spec = std::env::var("PAGED_TEST_DEVICE").unwrap_or_else(|_| "cpu".into());
    let device = resolve_device(&spec).unwrap();
    let tok = load_tokenizer_from_gguf(path).unwrap();
    let (eos_id, _, _, _) = read_gguf_meta(path, &tok).unwrap();
    let kv_blocks = std::env::var("PAGED_KV_BLOCKS").ok().and_then(|s| s.parse().ok()).unwrap_or(512usize);
    let engine = Engine::spawn(path, device, EngineConfig {
        max_num_seqs: cap, block_size: 16, kv_cache_blocks: kv_blocks, max_seq: 4096, eos_token_id: eos_id,
    }).unwrap();
    let mut rxs = Vec::new();
    for p in prompts {
